...
Hacked together by / Copyrigsstmsssht 2020, Ross Wightman
"""
import copy
import logging
import math
    ...
    def forward(self, x: torch.Tensor, n_layers=0, n_executed_layers=0, layer_idx=0) -> torch.Tensor:
        ...
        if self.layer_router:
            ...
            if self.training:
                ...
                router_indices = torch.where(router_x > 0)[0]
                # d = time_fn(True)
                ...
                # f = time_fn(True)
                # x_routed = x_routed + self.drop_path1(self.ls1(self.attn(self.norm1(x_routed))))
                # x_pre[router_indices] = x_routed + self.drop_path2(self.ls2(self.mlp(self.norm2(x_routed)))) * router_x.view(len(router_x), 1, 1)
                x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x))))
                x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x)))) * router_x.view(len(router_x), 1, 1).expand_as(x)
                # g = time_fn(True)
                # print(
                #     "{0:.8f},{1:.8f},{2:.8f},{3:.8f},{4:.8f},{5:.8f},{6:.8f} before_sum{7:.8f} forward:{8:.8f}, total:{9:.8f}".format(a-a_1, b - a, c - b, d - c, e - d, f - e, g - f, d-a_1, g-f, g-a_1))
            else:
                # print("{0:.2f}us,{1:.2f}us,total:{2:.2f}us".format((t1-t0)*10**6,(t2-t1)*10**6, (t2-t0)*10**6))
                # t3 = time_fn(True)
                router_x = router_sigmoid(router_x * self.layer_router_lam)
                # t4 = time_fn(True)
                # print("{0:.2f}us, {1:.2f}us".format((t4-t3)*10**6, (t4-t3+t2-t0)*10**6))
            ...
            # t2 = time_fn(True)
            # print(f"{(t1 - t0) * 10 ** 3:.3f} ms, {(t2 - t1) * 10 ** 3:.3f} ms")
            return x
...
@register_model
def early_vit_tiny_patch16_224_gap(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=6, num_heads=3, global_pool='avg', fc_norm=False)
    model = _create_vision_transformer('vit_tiny_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def vit_small_patch16_224_depth6(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=6, num_heads=6, class_token=False, global_pool='avg')
...
def vit_small_patch16_224_depth12(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, class_token=False, global_pool='avg')
    model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def vit_base_patch16_224_depth6(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=6, num_heads=12, class_token=False, global_pool='avg')
    model = _create_vision_transformer('vit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def router_vit_small_patch16_224_depth12_6(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, layer_router=True,
                      layer_router_lam=5.0, layer_router_thre=0.5, class_token=False, global_pool='avg', num_router_expert=1)
    model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
def router_vit_small_patch16_224_depth12_6_e256(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, layer_router=True,
                      layer_router_lam=5.0, layer_router_thre=0.5, class_token=False, global_pool='avg', num_router_expert=256)
    model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def router_vit_small_patch16_224_depth12_6_e512(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, layer_router=True,
                      layer_router_lam=5.0, layer_router_thre=0.5, class_token=False, global_pool='avg', num_router_expert=512)
    model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def router_vit_small_patch16_224_depth12_6_e384(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, layer_router=True,
                      layer_router_lam=5.0, layer_router_thre=0.5, class_token=False, global_pool='avg', num_router_expert=384)
    model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def vit_tiny_patch16_224_cifar100(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3, num_classes=100)
...
if __name__ == '__main__':
    ...
    x = ((torch.rand(1, 3, 224, 224) - 0.5) / 0.5).cuda()
    # y = torch.randint(0, 1000, (512,)).cuda()  # ?ˆì‹œ???´ëž˜??1000ê°?
    model = router_vit_small_patch16_224_depth12_6_e256(pretrained=False).cuda()
    weights = torch.load(
        '/home/hun/private/layer_router_ryu/output/train/router_vit_small_patch16_224_depth12_6_e256_20250509_220820/model_best.pth.tar')
    ...
    #         mlp_inputs[f'block_{i}'] = output.detach()
    #
    #     return hook_fn
    #
    #
    ...
    #     handles.append(block.norm2.register_forward_hook(get_hook(i)))
    #
    # output = model(x)
    #
    # for i in range(len(model.blocks)):
    #     mlp_input = mlp_inputs.get(f'block_{i}')
    #     if mlp_input is not None:
    #         mlp_output = model.blocks[i].mlp(mlp_input)
    #         print(f"Block {i} MLP output std: {mlp_output.std().item()}")
    #
    ...
    # loss.backward()  # ???´ê²Œ ?†ìœ¼ë©?grad ?„ë? None
    #
    # # Gradient norm ì²´í¬
    # total_norm = 0
