...
def router_sigmoid(x):
    return torch.sigmoid(torch.clamp(x, min=-10.0, max=10.0))
...
class Block(nn.Module):
    def __init__(
        ...
        self.partial_router = partial_router
        self.aux_bce_loss = aux_bce_loss

        if layer_router:
            self.router_fc = nn.Conv2d(dim, num_router_expert, kernel_size=1)
            self.router_norm = routerNormalize(layer_router_thre)
            self.eps = 1e-6
            if self.aux_bce_loss:
                self.bce_loss = nn.BCEWithLogitsLoss()

    def forward(self, x: torch.Tensor, n_layers=0, n_executed_layers=0, layer_idx=0) -> torch.Tensor:
        time_fn = partial(cuda_timestamp, device=x.device)
        if self.layer_router:
            router_x = self.router_fc(x.mean(dim=1).unsqueeze(-1).unsqueeze(-1)).squeeze()
            t0 = time_fn(True)
            if self.training:
                if self.num_router_expert > 1:
                    indices = torch.randint(0, self.num_router_expert, (router_x.size()[0],))
                    router_x = router_x[torch.arange(router_x.size()[0]), indices]
                ...
                if self.num_router_expert > 1:
                    if len(router_x.size()) == 1:
                        router_x = router_x.reshape(1, -1) # 1, E
                    router_x = router_x.mean(dim=1, keepdim=True)
                router_x = self.router_norm(router_x).squeeze(-1)
            t1 = time_fn(True)

            if self.aux_bce_loss:
                router_y = torch.where(router_x < 0.0, torch.tensor(0.0, dtype=router_x.dtype).cuda(),
                                       torch.tensor(1.0, dtype=router_x.dtype).cuda())
                aux_loss = self.bce_loss(router_x * self.layer_router_lam, router_y)

            router_x = router_sigmoid(router_x * self.layer_router_lam)
            t2 = time_fn(True)

            if self.training:
                router_x = torch.where(router_x < 0.5, torch.zeros_like(router_x), router_x)
                layer_idx = layer_idx + 1

                n_layers += router_x.size()[0]
                router_indices = (router_x > 0).nonzero(as_tuple=True)[0]
                n_executed_layers += len(router_indices)

                if self.partial_router == 'attention':
                    x_pre = x.clone()
                    if len(router_indices) <= 0:
                        pass
                    else:
                        x = x[router_indices, :].contiguous()
                        router_x = router_x[router_indices].contiguous()
                        x_pre[router_indices, :] = x + self.drop_path1(self.ls1(self.attn(self.norm1(x)))) * router_x.view(len(router_x), 1, 1)
                    x_pre = x_pre + self.drop_path2(self.ls2(self.mlp(self.norm2(x_pre))))

                else:
                    # self.partial_router == 'mlp'
                    x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x))))

                    if len(router_indices) <= 0:
                        # [수정] 기존: return x, n_layers, n_executed_layers, layer_idx
                        # nn.Sequential에서 호출 시 튜플 반환하면 다음 블록에서 크래시 발생하므로 텐서만 반환
                        return x

                    x_pre = x.clone()
                    x = x[router_indices, :].contiguous()
                    router_x = router_x[router_indices].contiguous()
                    x_pre[router_indices, :] = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x)))) * router_x.view(len(router_x), 1, 1)

                    # print("changed...")
                    # x_pre = x
                    # x_routed = x[router_indices]
                    # router_x = router_x[router_indices]
                    # x_pre[router_indices] = x_pre[router_indices] + self.drop_path2(self.ls2(self.mlp(self.norm2(x_routed)))) * router_x.view(len(router_x), 1, 1)
                # [수정] x는 router_indices로 축소된 텐서이므로, 전체 배치 결과가 담긴 x_pre를 x에 복원
                # 이 줄이 없으면 return x가 축소된 텐서를 반환하여 다음 블록에서 shape 불일치 발생
                x = x_pre
            else:
                # print("### before attn layer MEM allocated:", torch.cuda.memory_allocated() / 1024 ** 2, "MB")
                # print("Max allocated:", torch.cuda.max_memory_allocated() / 1024 ** 2, "MB")
                x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x))))
                t3 = time_fn(True)

                # print("### after attn layer MEM allocated:", torch.cuda.memory_allocated() / 1024 ** 2, "MB")
                # print("Max allocated:", torch.cuda.max_memory_allocated() / 1024 ** 2, "MB")
                ...
                if x.shape[0] > 1:
                    ...
                    if router_x.item() < 0.5:
                        print('pass layer')
                        return x

                    # print("*** before mlp layer MEM allocated:", torch.cuda.memory_allocated() / 1024 ** 2, "MB")
                    # print("Max allocated:", torch.cuda.max_memory_allocated() / 1024 ** 2, "MB")
                    x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x)))) * router_x.view(len(router_x), 1, 1)
                    t4 = time_fn(True)
                    print("router compute {0:.2f}us, router sigmoid {1:.2f}us, attn {2:.2f}us, mlp {3:.2f}us".format((t1 - t0) * 10 ** 6, (t2 - t1) * 10 ** 6, (t3 - t2) * 10 ** 6, (t4-t3) * 10 ** 6))
                    # print("*** after mlp layer MEM allocated:", torch.cuda.memory_allocated() / 1024 ** 2, "MB")
                    # print("Max allocated:", torch.cuda.max_memory_allocated() / 1024 ** 2, "MB")
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            aux_bce_loss = False,
            alpha=0,
            default_last_layer=False,
            default_first_layer=False,
    ) -> None:
        """
        Args:
        ...
        # additional init for layer_router
        self.layer_router = layer_router
        self.aux_bce_loss = aux_bce_loss
        self.alpha = alpha
        self.default_last_layer = default_last_layer
        self.default_first_layer = default_first_layer
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                act_layer=act_layer,
                mlp_layer=mlp_layer,
                layer_router=layer_router if not(default_last_layer and i == depth-1) and not(default_first_layer and i == 0) else False,
                layer_router_lam=layer_router_lam,
                layer_router_thre=layer_router_thre,
                layer_idx=i,
                num_router_expert=num_router_expert,
                partial_router=partial_router,
                aux_bce_loss=aux_bce_loss
            )
    ...
    def forward_features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.patch_drop(x)
        x = self.norm_pre(x)
        # print("before backbone MEM allocated:", torch.cuda.memory_allocated() / 1024 ** 2, "MB")
        # print("Max allocated:", torch.cuda.max_memory_allocated() / 1024 ** 2, "MB")
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if self.layer_router:
                ...
                #     if self.aux_bce_loss:
                #         x, n_layers, n_executed_layers, layer_idx, aux_loss = blk(x, n_layers, n_executed_layers, layer_idx)
                #         aux_losses += aux_loss
                #     else:
                #         if (self.default_last_layer and layer_idx == len(self.blocks) - 1) or (
                #                 self.default_first_layer and layer_idx == 0):
                #             x = blk(x, n_layers, n_executed_layers, layer_idx)
                #             n_layers += x.shape[0]
                #             n_executed_layers += x.shape[0]
                #             layer_idx += 1
                #         else:
                #             # print("before block MEM allocated:", torch.cuda.memory_allocated() / 1024 ** 2, "MB")
                #             # print("Max allocated:", torch.cuda.max_memory_allocated() / 1024 ** 2, "MB")
                #             t0 = time_fn(True)
                #             x, n_layers, n_executed_layers, layer_idx = blk(x, n_layers, n_executed_layers, layer_idx)
                #             t1 = time_fn(True)
                #             print(f"per layer {(t1-t0) * 10**3 :.3f} ms")
                #             total_time += (t1 - t0) * 10**3
                #             # print("after block MEM allocated:", torch.cuda.memory_allocated() / 1024 ** 2, "MB")
                #             # print("Max allocated:", torch.cuda.max_memory_allocated() / 1024 ** 2, "MB")
                # print(f"total_time of blocks: {total_time:.2f} ms")
                x = self.blocks(x)
            else:
                x = self.blocks(x)
        x = self.norm(x)
        # print("after backbone MEM allocated:", torch.cuda.memory_allocated() / 1024 ** 2, "MB")
        # print("Max allocated:", torch.cuda.max_memory_allocated() / 1024 ** 2, "MB")
        if self.aux_bce_loss:
            return x, aux_losses / len(self.blocks) * self.alpha
        return x
...
# @register_model
# def router_vit_small_patch16_224_depth12_6_e256(pretrained: bool = False, **kwargs) -> VisionTransformer:
#     """ ViT-Small (ViT-S/16)
#     """
#     model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, layer_router=True,
#                       layer_router_lam=5.0, layer_router_thre=0.5, class_token=False, global_pool='avg',
#                       num_router_expert=256)
#     model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
#     return model

# @register_model
# def vit_small_patch16_224_depth12(pretrained: bool = False, **kwargs) -> VisionTransformer:
#     """ ViT-Small (ViT-S/16)
#     """
...
@register_model
def router_vit_small_patch16_224_depth12_6_e256_partial_attention(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, layer_router=True,
                      layer_router_lam=5.0, layer_router_thre=0.5, class_token=False, global_pool='avg',
                      num_router_expert=256, partial_router='attention')
    model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def router_vit_small_patch16_224_depth12_6_e256_partial_attention_auxBCELoss(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, layer_router=True,
                      layer_router_lam=5.0, layer_router_thre=0.5, class_token=False, global_pool='avg',
                      num_router_expert=256, partial_router='attention', aux_bce_loss=True, alpha=0.01)
    model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def router_vit_small_patch16_224_depth12_6_e256_partial_mlp(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, layer_router=True,
                      layer_router_lam=5.0, layer_router_thre=0.5, class_token=False, global_pool='avg',
                      num_router_expert=256, partial_router='mlp')
    model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def router_vit_small_patch16_224_depth12_8_e256_partial_mlp(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, layer_router=True,
                      layer_router_lam=5.0, layer_router_thre=0.5, class_token=False, global_pool='avg',
                      num_router_expert=256, partial_router='mlp', default_first_layer=True, default_last_layer=True)
    model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def router_vit_base_patch16_224_depth12_6_e256_partial_mlp(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, layer_router=True,
                      layer_router_lam=5.0, layer_router_thre=0.5, class_token=False, global_pool='avg',
                      num_router_expert=256, partial_router='mlp')
...
if __name__ == '__main__':
    ...
    x = ((torch.rand(512, 3, 224, 224) - 0.5) / 0.5).cuda()
    # y = torch.randint(0, 1000, (512,)).cuda()  # 예시용 클래스 1000개

    model = router_vit_small_patch16_224_depth12_8_e256_partial_mlp().cuda()
    # model = router_vit_small_patch16_224_depth12_6_e256_partial_mlp().cuda()
    ...
    # y = torch.randint(0, 1000, (512,)).cuda()  # 예시용 클래스 1000개
    # weights = torch.load(
        # '/home/hun/private/layer_router_ryu/output/train/router_vit_small_patch16_224_depth12_6_e256_partial_attention_20250618_135339/last.pth.tar')
    # weights = torch.load('/home/hun/private/layer_router_ryu/last.pth.tar')
    # model.load_state_dict(weights['state_dict'])

    # for i in range(12):
    #     print('layer', i)
    #     median, std = f'blocks.{i}.router_norm.running_median', f'blocks.{i}.router_norm.running_std'
    #     print(weights['state_dict'][median], weights['state_dict'][std])
    ...
    #         _inputs[f'block_{i}'] = output.detach()
    #
    #     return hook_fn
    # for i, block in enumerate(model.blocks):
    #     handles.append(block.norm1.register_forward_hook(get_hook(i)))
    #
    # output = model(x)
    #
    # for i in range(len(model.blocks)):
    #     _input = _inputs.get(f'block_{i}')
    #     if _input is not None:
    #         _output = model.blocks[i].attn(_input)
    #         print(f"Block {i} Attn/MLP output std: {_output.std().item()}")
    #
    ...
    # loss.backward()  # ← 이게 없으면 grad 전부 None
    #
    # # Gradient norm 체크
    # total_norm = 0
    ...
    #             max_param = name
    #
    # print(f"Largest grad norm: {max_norm:.4f} at {max_param}")