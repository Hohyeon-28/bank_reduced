...
# Joint utility-topk PatternMLP ViT experiment.
#
# Instead of selecting a predefined pattern from a bank, the router predicts
# per-layer MLP utility scores and enables the top-k MLP residual branches for
# each sample.
...
UTILITY_TOPK="${UTILITY_TOPK:-6}"
TARGET_BUDGET="${TARGET_BUDGET:-${UTILITY_TOPK}}"
...
echo "UTILITY_TOPK=${UTILITY_TOPK}"
...
echo "[Joint] Utility top-k router + ViT training"
torchrun --nproc_per_node="${NPROC}" train_sh.py "${DATA_DIR}" \
  ...
  --model-kwargs \
    pattern_mode=utility_topk \
    utility_topk_k="${UTILITY_TOPK}" \
    router_init=default \
