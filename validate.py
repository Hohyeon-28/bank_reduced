...
from models.early_vit import router_vit_small_patch16_224_depth12_6_e256, vit_small_patch16_224_depth6
from models.early_vit_partial_routing import router_vit_small_patch16_224_depth12_6_e256_partial_mlp, router_vit_small_patch16_224_depth12_6_e256_partial_attention
from models.early_resnet import router_resnet50_block16_8_e256
from models.early_pattern_vit import router_vit_small_patch16_224_depth12_pattern_router
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if print_profile:
                ...
                if print_profile:
                    ...
                    flops, macs = flops / 10 ** 9, macs / 10 ** 9
                    flops = flops / input.size(0)
                    macs = macs / input.size(0)
                    profile_list.append(macs)

                prof.end_profile()

            # througput check
            total_step += step_end - step_start
            n_samples += input.size(0)
            # if batch_idx % args.log_freq == 0:
            #     print(n_samples, total_step)
            #     print(f" {n_samples / total_step:.2f} samples/sec.")

    # flops proflile
    if print_profile:
        print('FLOPs profile results')
        print('max:', max(profile_list), 'min:', min(profile_list), 'avg:', sum(profile_list) / len(profile_list))
